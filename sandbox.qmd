...
1. Read data from an external API, Star Wars ✅  - API IS DOWN!
...
\
Read data from an external API, Weather forecast in NYC, API: [The National Weather Service (NWS)](https://api.weather.gov) - ERROR: there is no package called 'httr2', 'lubridate' ???
```{r}
#| echo: true
#| warning: false
#| eval: true
...
extracted_data |> gt::gt() |> gt::opt_interactive(page_size_default = 10)
```
